...
impl Layer for YoloLayer {
    ...
    fn infer_shape(&mut self, input_shapes: Vec<Rc<dyn Shape>>) -> Result<(), ShapeError> {
        if input_shapes.len() != 1 {
            return Err(ShapeError{description: String::from("YoloLayer must have exact one input layer")})
        }
        let input_shape = &input_shapes[0];
        if input_shape.dims().len() != 4 {
            return Err(ShapeError{description: String::from("YoloLayer can be connected only with layer, which produce 4D Tensor with format NCHW")})
        }
